...
static bool update_step_result(uint32_t input_time_stamp_ms)
{
    ...
    mArg.current_step_count_result = mArg.prev_accumulated_step_count + get_pedometer_step_count();
    if (mArg.current_step_count_result != mArg.prev_step_count_result) {
        ...
        update_result = true;
        osLog(LOG_INFO, "STEP_COUNTER RESULT UPDATE: dirver=%lu, algo=%lu @ %llu\n", accumulated_step_count,
              get_pedometer_step_count(), rtcGetTime());
    }

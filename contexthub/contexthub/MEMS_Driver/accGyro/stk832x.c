...
#define STK832X_VERSION     "0.2.10-1"
...
//arvin_liao add
#ifdef CFG_SENSORHUB_PRIZE_HARDWARE_INFO
static void stkGetSensorHardwareInfo(struct sensor_hardware_info_t *data)
{
	memcpy(data, &mTask.dev_hardware_info, sizeof(struct sensor_hardware_info_t));
	osLog(LOG_ERROR, "stkGetSensorHardwareInfo chip:%s id:%s more:%s vendor:%s \n", data->chip, data->id, data->more, data->vendor);
}
#endif
...
//arvin_liao add
#ifdef CFG_SENSORHUB_PRIZE_HARDWARE_INFO
	mInfo.getSensorHardwareInfo = stkGetSensorHardwareInfo;
#endif
...
//        sensorFsmEnqueueFakeSpiEvt(spiCallBack, STATE_ACC_RATECHG_DONE, ERROR_EVT);
    ...
    osLog(LOG_INFO, "%s acc rate:%d, latency:%lld, watermark:%d \n",
                           ...
                           void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    ...
    if (0 < mTask.watermark)
    {
        ...
        {
            //osLog(LOG_INFO, "Detected FIFO int \n");
...
static void stkParseRawData(struct accGyroData *data, uint8_t *buf)
{
    ...
    if (mTask.hd_status)
    {
        if (stk_hand_detect((int *)remap_data))
        {
            mTask.hd_cnt++;
        }
        else if (0 < mTask.hd_cnt)
        {
            mTask.hd_cnt--;
        }
        else
        ...
        {
            osLog(LOG_INFO, "Detected hand detect \n");
            mTask.hd_cnt = 0;
            mTask.hd_status = false;
        }
    }
    ...
    if (UNLIKELY(mTask.startCali))
    {
        ...
        if (0 != status)
        {
            ...
            {
                osLog(LOG_ERROR, "ACC cali time out, status=%d \n", status);
            }
        }
        else if (3 == accuracy)
        {
            ...
            accGyroSendCalibrationResult(SENS_TYPE_ACCEL,
                                      (int32_t *)&caliResult[0], (uint8_t)status);
#endif /* no STK_ENV_LACK */
            osLog(LOG_INFO, "ACC cali done, caliResult:%d %d %d, offset:%f %f %f \n",
                         ...
                         void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
	...
	//arvin_liao add for fix acc cali return fail cased by too fast odr
	SPI_WRITE(STK832X_REG_BWSEL, STK832X_BWSEL_BW_250, 1024000000 / mTask.sampleRate);
    return spiBatchTxRx(&mTask.spiMode, spiCallBack, next_state, __FUNCTION__);
    //sensorFsmEnqueueFakeSpiEvt(spiCallBack, next_state, SUCCESS_EVT);
    //return 0;
}
                            ...
                            void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    ...
    osLog(LOG_INFO, "%s, cfgData[0]:%d, cfgData[1]:%d, cfgData[2]:%d \n",
          __func__, caliCfgPacket.caliCfgData[0], caliCfgPacket.caliCfgData[1], caliCfgPacket.caliCfgData[2]);
#endif /* STK_DEBUG_MODE */
...
//arvin_liao add	
#ifdef CFG_SENSORHUB_PRIZE_HARDWARE_INFO
	strncpy(mTask.dev_hardware_info.chip, "stk832x", sizeof(mTask.dev_hardware_info.chip) - 1);
	strncpy(mTask.dev_hardware_info.vendor, "Sensortek", sizeof(mTask.dev_hardware_info.vendor) - 1);
	snprintf(mTask.dev_hardware_info.id,  sizeof(mTask.dev_hardware_info.id) - 1, "spi%d", mTask.hw->i2c_num);
	strncpy(mTask.dev_hardware_info.more, "accelerometer", sizeof(mTask.dev_hardware_info.more) - 1);
#endif
    return ret;
exit_error:
